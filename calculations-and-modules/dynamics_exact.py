...
for i in range(len(t_list)):
    ...
    ori /= np.trace(ori)
    exact_soln.append(ori)
    
GS_ex = [i[0, 0].real for i in exact_soln]
ES_ex = [i[1, 1].real for i in exact_soln]

np.save('GS_pop_exact'+str(B)+'T'+str(temperature)+'K.npy', GS_ex)
np.save('ES_pop_exact'+str(B)+'T'+str(temperature)+'K.npy', ES_ex)